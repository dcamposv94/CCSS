...
Class ClasePaciente 
{
    ...
    function ListarPaciente() {
        ...
        $sql = "SELECT * FROM tbl_pacientes"; // se arma el query

        $resultado = $BD->query($sql); // Se ejecuta el query
        //Se valida que el query haya retornado usuarios
        if ($resultado->num_rows != 0) {
            $pacientes = array(); //Se crea arreglo para almancenar usuarios
            ...
            while ($registro = $resultado->fetch_assoc()) {
                ...
                array_push($pacientes, $registro);
            }
            $retorno["valido"] = true;
            $retorno["pacientes"] = $pacientes;
        } else {
